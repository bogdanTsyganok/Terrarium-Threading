...
{
	this->m_maze_size[0] = width + 1;
	this->m_maze_size[1] = height + 1;
	...
	// Make 6 empty areas
	this->m_ClearArea(10, 20, 10, 20);
	this->m_ClearArea(40, 50, 10, 20);
	
	//this->m_ClearArea(10, 20, 45, 55);
	//this->m_ClearArea(40, 50, 45, 55);

	/*this->m_ClearArea(10, 20, 80, 90);
	this->m_ClearArea(40, 50, 80, 90);*/

	return true;
}

void cMazeMaker::m_ClearArea(unsigned int wStart, unsigned int wEnd, unsigned int hStart, unsigned int hEnd)
{
	for ( unsigned int wIndex = wStart; wIndex != wEnd; wIndex++ )
	{
		for ( unsigned int hIndex = hStart; hIndex != hEnd; hIndex++ )
		{
			this->maze[wIndex][hIndex][0] = false;
		}
	}
...
void cMazeMaker::PrintMazeToFile(std::string fileName)
{
	...
	unsigned int height = this->maze.size();
	unsigned int width = this->maze[0].size();

	for ( unsigned int a = 0; a < height; a++ )
	{
		for ( unsigned int b = 0; b < width; b++ )
		{
			// Bug from original posting, where columns and rows are switched.
			if ( this->maze[height-a-1][b][0] )
				//			if (this->maze[b][a][0])
			{
				theFile << "X";
				//				std::cout << "X";
